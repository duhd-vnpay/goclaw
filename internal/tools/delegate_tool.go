...
func (t *DelegateTool) Execute(ctx context.Context, args map[string]any) *Result {
	...
	// Reject an explicitly-requested async child mode before any lookup: inside a
	// delegation artifact run only "sync" is allowed. An omitted mode is validated
	// again after per-link default resolution below (a link may default to sync).
	if explicitMode != "" {
		if err := validateDelegationChildRunMode(ctx, "delegate", explicitMode); err != nil {
	...
	// Validate the resolved effective mode (per-link default may make it "sync"
	// inside an artifact run, which is the only mode allowed there).
...
func (t *DelegateTool) executeAsyncMode(ctx context.Context, job *delegateArtifactJob) *Result {
	...
	// VNPAY fork: 20min ceiling (upstream 10min) — reasoning subagents (extended
	// thinking + 15-20KB JS gen) can need 5-8min per iter × 6-10 iter for
	// parse+render+validate+send workflows.
