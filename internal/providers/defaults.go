...
func NewDefaultTransport() *http.Transport {
	return &http.Transport{
		...
		// multiplies it again.
		//
		// VNPAY fork keeps 600s (upstream: 300s): our provider hop goes through
		// LiteLLM/CLIProxyAPI whose LB is configured for a 600s LLM timeout, and
		// reasoning models with 16K+ input + thinking routinely emit nothing for
		// 5-8min before the first byte. A shorter deadline killed healthy requests
		// mid-thought (upstream saw the same at 180s → raised to 300s; our infra
		// needs the full 600s to match the LB bound).
		//
		...
		// no run-level deadline to fall back on. Without this timeout a silently
		// dead connection would hold its scheduler lane until the OS TCP keepalive
		// gave up (~2h), and requeue could not rescue it because requeue only fires
		// when a run ends.
