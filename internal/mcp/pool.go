...
// Increments the reference count. Blocks up to AcquireTimeout if pool is at MaxSize.
// key is the composite key (e.g. "tenantID/name" or "tenantID/name:projectID") used for
// process isolation; name is the server name used for connectAndDiscover.
func (p *Pool) Acquire(ctx context.Context, key, name, transportType, command string, args []string, env map[string]string, url string, headers map[string]string, timeoutSec int) (*poolEntry, error) {
	p.mu.Lock()
...
// Accepts the same key format as Acquire.
