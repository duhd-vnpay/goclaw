...
func TestResponseHeaderTimeoutAccommodatesReasoningModels(t *testing.T) {
	...
	// VNPAY fork sets 600s (upstream 300s): our provider hop goes through
	// LiteLLM/CLIProxyAPI whose LB is configured for a 600s LLM timeout, and
	// reasoning models with 16K+ input routinely need 5-8min before first byte.
	// 3 (retry) x 3 (requeue) x 600s = 90min worst case — the ceiling this fork
	// value is chosen against, in exchange for matching the LB bound.
	const maxAcceptable = 600 * time.Second
